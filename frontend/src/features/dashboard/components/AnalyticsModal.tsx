...
export const AnalyticsModal = ({ isOpen, onClose, userId }: AnalyticsModalProps) => {
    ...
    const ChartBar = ({ date, output, score, maxOutput }: { date: string, output: number, score: number, maxOutput: number }) => {
        ...
        return (
            <div className="flex flex-col items-center gap-2 group">
                <div className="relative w-12 h-48 bg-white/5 rounded-t-lg flex items-end justify-center overflow-hidden">
                    {/* Output Bar */}
                    <motion.div
                        initial={{ height: 0 }}
                    ...
                    {/* Tooltip */}
                    <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-black/90 border border-white/10 p-2 rounded-lg text-xs opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10 pointer-events-none">
                        <div className="flex items-center gap-2">
                            <Zap className="w-3 h-3 text-accent" /> {Math.round(output)}m
                        </div>
                        <div className="flex items-center gap-2">
                            <Activity className="w-3 h-3 text-white" /> Score: {score.toFixed(1)}/10
                        </div>
                    </div>
                </div>
                <span className="text-[10px] text-white/30 font-mono rotate-45 origin-left translate-y-2">
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-4xl bg-black/90 border border-white/10 backdrop-blur-xl rounded-3xl p-8 text-white">
                ...
                {loading ? (
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="grid grid-cols-3 gap-4">
                            <div className="bg-white/5 rounded-2xl p-4 border border-white/5">
                                <p className="text-xs text-white/40 uppercase tracking-widest mb-1">Total Focus</p>
                                <p className="text-2xl font-bold text-white">{data.summary?.total_hours || 0} <span className="text-sm font-normal text-white/40">hrs</span></p>
                            </div>
                            <div className="bg-white/5 rounded-2xl p-4 border border-white/5">
                                <p className="text-xs text-white/40 uppercase tracking-widest mb-1">Avg Session</p>
                                <p className="text-2xl font-bold text-white">{data.summary?.avg_session_mins || 0} <span className="text-sm font-normal text-white/40">mins</span></p>
                            </div>
                            <div className="bg-white/5 rounded-2xl p-4 border border-white/5">
                                <p className="text-xs text-white/40 uppercase tracking-widest mb-1">Sessions</p>
                                <p className="text-2xl font-bold text-white">{data.summary?.total_entries || 0}</p>
                            </div>
                        </div>
                        ...
                        <div className="bg-white/5 rounded-3xl p-8 border border-white/5">
                            ...
                            <div className="flex justify-between items-center mt-4 text-xs text-white/30 px-4">
                                <div className="flex items-center gap-2">
                    ...
                    <div className="h-64 flex flex-col items-center justify-center text-center border-2 border-dashed border-white/10 rounded-3xl">
                        <Calendar className="w-10 h-10 text-white/10 mb-4" />
                        <p className="text-white/40 font-medium">No Data Available</p>
                        <p className="text-xs text-white/20 mt-1">Complete a few sessions to see your trends.</p>
                    </div>
                )}
            </DialogContent>
        </Dialog>
