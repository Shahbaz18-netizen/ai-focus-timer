    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-bold uppercase tracking-widest text-foreground/50">Ambience</span>
                <button
                    onClick={toggleAudio}
                    className={`p-2 rounded-full transition-colors ${audioEnabled ? 'bg-accent/20 text-accent' : 'bg-panel text-foreground/40'}`}
                >
                ...
                {SOUND_STEMS.map((stem) => {
                    ...
                    return (
                        <div key={stem.id} className="flex items-center gap-3 sm:gap-4 group">
                            <Icon className={`w-4 h-4 shrink-0 transition-colors ${vol > 0 && audioEnabled ? 'text-accent' : 'text-foreground/30'}`} />
                            <div className="flex-1 relative h-8 flex items-center min-w-0">
                                <input
                                    ...
                                    className="w-full h-1 bg-panel-hover rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-accent [&::-webkit-slider-thumb]:opacity-0 group-hover:[&::-webkit-slider-thumb]:opacity-100 hover:[&::-webkit-slider-thumb]:scale-125 transition-all relative z-10"
                                />
                                <div
