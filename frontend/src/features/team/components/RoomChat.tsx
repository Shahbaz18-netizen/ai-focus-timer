...
export const RoomChat = ({ messages, onSendMessage, currentUserId }: RoomChatProps) => {
    ...
    return (
        <div className="flex flex-col h-full bg-background/20 rounded-xl border border-white/5 overflow-hidden">
            <div className="flex-1 overflow-y-auto p-4 space-y-4" ref={scrollRef}>
                <AnimatePresence initial={false}>
                    {messages.map((msg) => (
                        ...
                        >
                            <span className="text-[10px] text-foreground/30 ml-1 mb-1">{msg.username}</span>
                            <div
                                className={`px-3 py-2 rounded-2xl text-sm max-w-[80%] break-words ${msg.user_id === currentUserId
                                        ...
                                        : 'bg-panel-hover text-foreground border border-border-subtle rounded-bl-none'
                                    }`}
                            >
                                {msg.content}
                            </div>
            ...
            <form onSubmit={handleSend} className="p-3 border-t border-white/5 bg-white/2 flex gap-2">
                <input
                    ...
                    className="flex-1 bg-transparent border-none outline-none text-sm text-foreground placeholder:text-foreground/20"
                />
                <button
                    type="submit"
                    disabled={!input.trim()}
